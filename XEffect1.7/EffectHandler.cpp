...
void WriteImage(irr::video::IVideoDriver* driver, irr::video::ITexture* tex, const char* fileName)
{
	auto img = driver->createImageFromData(tex->getColorFormat(), tex->getSize(), tex->lock(), false, false);
	driver->writeImageToFile(img, fileName);
	tex->unlock();
}
